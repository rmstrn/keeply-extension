...
const addEvent = ({ name, parameters }, target) => {
    const event = createEvent((...args) => {
        if (args.length > parameters.length) {
            throw new Error(`Max number of arguments for ${name}.addListener is ${parameters.length}`);
        }
        // const types = [
        //   'boolean',
        //   'number',
...
function addStorageArea() {
    return {
        clear: vitest.vi.fn(),
        get: vitest.vi.fn(),
        getBytesInUse: vitest.vi.fn(),
        remove: vitest.vi.fn(),
        set: vitest.vi.fn(),
    };
}

function createHandler(schema = vitestChromeSchema) {
    return {
        ownKeys() {
